...
describe('Row components', () => {
	it('ProductRow renders update link (positive)', () => {
		...
		// link rendered by NavigationLink but we can search by role=link
	...
	it('ProductRow does not crash without optional props (negative)', () => {
		render(
			<table>
				<tbody>
					{/* @ts-expect-error intentionally omitting optional fields */}
					<ProductRow id={'p2'} title={'X'} serialNumber={'S'} />
				</tbody>
			</table>
		);
